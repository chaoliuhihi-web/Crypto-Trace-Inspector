...
- Android：ADB 设备识别 + `pm list packages` 应用包清单（需要 USB 调试授权）
- iOS：配对验证 + 应用清单 + 可选 `idevicebackup2` 全量备份（需要信任/配对/备份授权）
- iOS 备份解析（新接入）：
  - 在备份可读前提下，尝试从 `Manifest.db` 定位 Safari `History.db` 并解析为 `browser_history` artifact
...
- 当前 Android 只采集应用包清单；浏览历史采集仍未实现（需要明确可行数据源与授权前置条件）。

### 3.2 iOS 备份解析真机验证

- 已实现 Safari `History.db` 解析逻辑，但需要在真实 iOS 设备完成备份后验证：
  - Manifest.db 表结构差异
  - History.db 表结构差异
  - 加密备份的处理策略（不做绕过）
...
当前版本已形成“主机采集 -> 命中 -> 证据 -> 审计 -> 报告 -> 司法导出 ZIP -> 离线校验”的闭环，并补齐了链上余额查询（含写入证据链的留痕能力）。移动端交易所访问检测已接入 iOS 备份解析路径，但仍需要真机/授权条件下验证与完善（尤其是 Android 浏览历史采集路径）。
